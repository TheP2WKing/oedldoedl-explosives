...
public class BlockAnitmatterBomb extends ModBlockTNTBase {
	public BlockAnitmatterBomb(String modid, String name, CreativeTabs tab, SoundType sound, MapColor mapColor,
			int lightLevel) {
	...
	public ModEntityTNTBase createTNTEntity(World world, double x, double y, double z, EntityLivingBase ignitor) {
		return new EntityAntimatterBomb(world, x, y, z, ignitor);
	}
		...
		if (!worldIn.isRemote) {
			if (((Boolean) state.getValue(EXPLODE)).booleanValue()) {
				...
				worldIn.playSound((EntityPlayer) null, pos.getX(), pos.getY(), pos.getZ(),
						ModSounds.BOMB, SoundCategory.BLOCKS, 5.0F, 1.0F);
			}
		}
	}
}