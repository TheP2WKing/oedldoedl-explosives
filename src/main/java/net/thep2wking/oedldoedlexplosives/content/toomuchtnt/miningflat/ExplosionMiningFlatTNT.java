...
public class ExplosionMiningFlatTNT extends ModExplosionBase {
	public ExplosionMiningFlatTNT(World worldIn, Entity ignitor, double x, double y, double z, float explosionStrength,
			...
			for (int k = 0; k < this.smoothness; ++k) {
				for (int l = 0; l < this.smoothness; ++l) {
					if (j == 0 || j == (this.smoothness - 1) || k == 0 || k == (this.smoothness - 1) || l == 0
						...
						double d1 = (double) ((float) k / (this.smoothness - 1) * 2.0F - 0.0F);
		...
		if (this.spawnFire) {
			for (BlockPos blockpos1 : this.affectedBlockPositions) {
				if (this.world.getBlockState(blockpos1).getMaterial() == Material.AIR
						&& this.world.getBlockState(blockpos1.down()).isFullBlock() && this.random.nextInt(30) == 0) {
