...
public class ExplosionSnow extends ModExplosionBase {
	public ExplosionSnow(World worldIn, Entity ignitor, double x, double y, double z, float explosionStrength,
		...
		if (this.spawnFire) {
			for (BlockPos blockpos1 : this.affectedBlockPositions) {
				...
				if (blockState.getMaterial() == Material.WATER) {
					...
					this.world.setBlockState(blockpos1, Blocks.OBSIDIAN.getDefaultState());
					...
					this.world.setBlockState(blockpos1, Blocks.SNOW_LAYER.getDefaultState());
				}
			}
		}
	}
				...
				for (int l = 0; l < this.smoothness; ++l) {
					if (j == 0 || j == (this.smoothness - 1) || k == 0 || k == (this.smoothness - 1) || l == 0
						...
						float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
							...
							if (iblockstate.getMaterial() == Material.AIR || iblockstate.getBlock().isReplaceable(this.world, blockpos)) {
								set.add(blockpos);
							}
