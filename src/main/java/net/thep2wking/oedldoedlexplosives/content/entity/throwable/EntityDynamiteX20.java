...
public class EntityDynamiteX20 extends EntityThrowable {
	public EntityDynamiteX20(World worldIn) {
		super(worldIn);
	}

	public EntityDynamiteX20(World worldIn, EntityLivingBase throwerIn) {
		super(worldIn, throwerIn);
	}

	public EntityDynamiteX20(World worldIn, double x, double y, double z) {
		super(worldIn, x, y, z);
	}
		...
		if (id == 3) {
			for (int i = 0; i < 8; ++i) {
				this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.posX, this.posY, this.posZ, 0.0D, 0.0D,
						0.0D, Item.getIdFromItem(ModItems.DYNAMITE_X20));
			}
		}
	}
	...
	public void onImpact(RayTraceResult result) {
		...
		if (!this.world.isRemote) {
			...
			this.world.newExplosion(null, this.getPosition().getX(), this.getPosition().getY(),
					this.getPosition().getZ(), 18.0f, false, true);
			this.setDead();
		}
	}
}