...
public class BlockMegaNukeTNT extends ModBlockTNTBase {
	public BlockMegaNukeTNT(String modid, String name, CreativeTabs tab, SoundType sound, MapColor mapColor,
			int lightLevel) {
	...
	public ModEntityTNTBase createTNTEntity(World world, double x, double y, double z, EntityLivingBase ignitor) {
		return new EntityMegaNukeTNT(world, x, y, z, ignitor);
	}
		...
		if (!worldIn.isRemote) {
			if (((Boolean) state.getValue(EXPLODE)).booleanValue()) {
				...
				worldIn.playSound((EntityPlayer) null, pos.getX(), pos.getY(), pos.getZ(),
						ModSounds.TACTICAL_NUKE, SoundCategory.BLOCKS, 5.0F, 1.0F);
			}
		}
	}
}